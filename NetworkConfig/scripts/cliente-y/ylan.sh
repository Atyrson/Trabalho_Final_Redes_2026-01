...
echo "=== [Y] Configuração da Interface de Rede e Gateway Padrão ==="
echo
...
read -rp "Endereço IP do Cliente X [padrão 192.168.0.3/24]: " IP_X
IP_X=${IP_X:-192.168.0.3/24}
...
echo -e "\n[Y] Desativando NetworkManager para evitar conflitos..."
sudo systemctl stop NetworkManager || true

echo "[Y] Limpando endereços antigos de $IF_X..."
sudo ip addr flush dev "$IF_X"

echo "[Y] Configurando endereço IP..."
sudo ip addr add "$IP_X" dev "$IF_X"
sudo ip link set "$IF_X" up
sudo ip link set "$IF_X" multicast on

echo "[Y] Configurando o Gateway Padrão (Rota para R2)..."
